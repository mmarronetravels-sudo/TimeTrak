...
const C = { navy: '#2c3e7e', green: '#16a34a', red: '#dc2626', orange: '#f3843e' };
...
export default function ReviewTimecards() {
  ...
  const loadTimecards = async () => {
    ...
    if (!staffList || staffList.length === 0) { setTimecards([]); return; }
    ...
    const { data: tcs } = await query;
    // Attach staff info
    const enriched = (tcs || []).map(tc => ({
      ...tc,
      staff: staffList.find(s => s.id === tc.staff_id),
    }));
    setTimecards(enriched);
  };
  ...
  const handleApprove = async (tc) => {
    await supabase.from('timecards').update({ status: 'supervisor_approved' }).eq('id', tc.id);
    ...
    for (const hr of (hrUsers || [])) {
      await supabase.from('notifications').insert({
        ...
        recipient_id: hr.id,
        type: 'timecard_supervisor_approved',
        title: 'Timecard Ready for HR Review',
        message: `${tc.staff?.full_name}'s timecard was approved by ${profile.full_name}`,
        related_id: tc.id,
      });
    }
    showNotif(`Timecard approved for ${tc.staff?.full_name}`);
    setExpanded(null);
    loadTimecards();
  };
  ...
  const handleReturn = async (tc) => {
    ...
    await supabase.from('notifications').insert({
      ...
      message: `Your timecard was returned by ${profile.full_name}. Please correct and resubmit.`,
      related_id: tc.id,
    });
    setReturnNote('');
    showNotif(`Timecard returned to ${tc.staff?.full_name}`);
    setExpanded(null);
    loadTimecards();
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-3">
        <div>
          <h2 className="text-2xl font-bold" style={{ color: C.navy }}>Review Timecards</h2>
          <p className="text-sm text-gray-500 mt-0.5">Your assigned staff</p>
        </div>
        <div className="flex gap-2">
          {['all', 'submitted', 'supervisor_approved', 'returned'].map(f => (
            <button
              key={f}
            ...
            >
              {f === 'all' ? 'All' : f === 'supervisor_approved' ? 'Approved' : f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-3">
        {timecards.map(tc => (
          <div key={tc.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            >
              <div>
                <p className="font-semibold text-gray-800">{tc.staff?.full_name}</p>
                <p className="text-xs text-gray-500">{tc.staff?.position} · {tc.staff?.building} · Week of {tc.week_start}</p>
              </div>
              ...
              <div className="border-t border-gray-100 px-5 py-4">
                ...
                {tc.status === 'submitted' && (
                  <div className="flex flex-col sm:flex-row gap-3 pt-3 border-t border-gray-100">
                    <button onClick={() => handleApprove(tc)} className="px-5 py-2 rounded-lg text-sm font-semibold text-white shadow-sm hover:shadow-md transition-all" style={{ background: C.green }}>
                      ✓ Approve
                    </button>
                    <div className="flex-1 flex gap-2">
                      <input
                        ...
                        className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#477fc1]"
                      />
